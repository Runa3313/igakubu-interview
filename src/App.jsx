...
const downloadPDF = async (univ, messages, allFeedbacks) => {
  ...
  const avgScore = allFeedbacks.length > 0
    ? Math.round(allFeedbacks.reduce((s, f) => s + (f.score || 0), 0) / allFeedbacks.length) : 0;
  const avgAdpoli = allFeedbacks.length > 0
    ? Math.round(allFeedbacks.reduce((s, f) => s + (f.adpoli_match || 0), 0) / allFeedbacks.length) : 0;
  ...
  const text = (str, x, size = 10, color = [30, 41, 59], bold = false) => {
    ...
    checkPage(lines.length * (size * 0.45 + 1));
    doc.text(lines, x, y);
  ...
  doc.text(`${univ.name} / ${univ.format} ${univ.duration}min / ${date}`, margin, 17);
  y = 28;
  ...
  [
    ...
    { label: "Adpoli Match", val: avgAdpoli },
    { label: "Questions", val: allFeedbacks.length },
  ].forEach((sb, i) => {
  ...
  y += 4;
  text(`AdPoli: ${univ.adpoli}`, margin + 3, 8, [100, 116, 139]);
  y += 4;
  ...
  pairs.forEach((pair, i) => {
    ...
    if (pair.fb) {
      doc.text(`Score: ${pair.fb.score}  Adpoli: ${pair.fb.adpoli_match}`, margin + contentW - 3, y + 5, { align: "right" });
    }
    y += 9;

    text(`[Q] ${pair.q.replace(/\n/g, " ")}`, margin + 2, 9, [71, 85, 105], true);
    y += 1;
    text(`[A] ${pair.a.replace(/\n/g, " ")}`, margin + 2, 9, [30, 41, 59]);
    y += 1;

    if (pair.fb) {
      if (pair.fb.good_points?.length > 0) {
        doc.setFontSize(8.5);
        doc.setFont("helvetica", "bold");
        doc.setTextColor(5, 150, 105);
        checkPage(6);
        doc.text("Good: " + pair.fb.good_points[0], margin + 2, y);
        y += 5;
      }
      if (pair.fb.improve_points?.length > 0) {
        doc.setFontSize(8.5);
        doc.setFont("helvetica", "bold");
        doc.setTextColor(220, 38, 38);
        checkPage(6);
        doc.text("Improve: " + pair.fb.improve_points[0], margin + 2, y);
        y += 5;
      }
      if (pair.fb.better_answer) {
        text(`Better: ${pair.fb.better_answer}`, margin + 2, 8.5, [124, 58, 237]);
      }
    }
...
- 学生の回答にツッコミどころ（矛盾・抽象的・根拠が薄い・具体性がない）があれば深掘りする追加質問をしてください。
- ツッコミどころがなければよく聞かれる質問リストから次の未回答の質問をしてください。
- ${isLastTurn ? `これが最後の質問です。必ず「${univ.closingQuestion}」と聞いてください。` : ""}
- 出力は面接官の質問文のみ（1〜2文）。前置き・「ありがとうございました」は禁止。終了の挨拶は最後のターン以外絶対禁止。`;
...
- 重視ポイント: ${univ.emphasis}
【スコア基準】
- 85〜100: 具体的エピソード・論理構成・アドポリ合致が全て揃っている
- 70〜84: 具体性があり内容も良いが、さらに深められる余地あり
- 55〜69: 方向性は合っているが具体性や根拠がやや不足
- 40〜54: 抽象的・アドポリとのズレあり・改善が必要
- 0〜39: 大幅に不足・的外れ・具体性なし
以下のJSON形式のみで返してください（コードブロック・説明文不要）:
{
  "score": 0〜100の整数（普通の回答は65〜70点、良い回答は75〜85点、とても良い回答は85点以上）,
  "good_points": ["良かった点を1〜2つ（具体的に）"],
  "improve_points": ["改善点を1〜2つ（具体的に）"],
  "better_answer": "もっと良い回答例（具体的なエピソードを含む2〜3文）",
    ...
    try {
      const [interviewerRes, feedbackRes] = await Promise.all([
        fetch("https://api.anthropic.com/v1/messages", {
          ...
          body: JSON.stringify({ model: "claude-sonnet-4-6", max_tokens: 300, system: interviewerSys, messages: interviewerMessages }),
        }),
        fetch("https://api.anthropic.com/v1/messages", {
          method: "POST", headers: API_HEADERS,
          body: JSON.stringify({ model: "claude-sonnet-4-6", max_tokens: 800, system: feedbackSys, messages: [{ role: "user", content: `質問：${lastQuestion}\n\n学生の回答：${studentText}` }] }),
        }),
      ]);
      ...
      try {
        ...
        if (start !== -1 && end !== -1) parsedFb = JSON.parse(rawFb.slice(start, end + 1));
      } catch { parsedFb = null; }

      if (parsedFb) {
        parsedFb._question = lastQuestion;
        parsedFb._answer = studentText;
        setFeedback(parsedFb);
        setAllFeedbacks(prev => [...prev, parsedFb]);
      }
      setFeedbackLoading(false);
  ...
  const retryLastQuestion = () => {
    ...
    setFeedback(null);
    setAllFeedbacks(prev => prev.slice(0, -1));
    setPhase("interview");
  };

  const resetAll = () => {
    setSelectedUniv(null); setPhase("select"); setMessages([]);
    setFeedback(null); setAllFeedbacks([]); setTurnCount(0);
    setInput(""); setShowSummary(false);
  };
            ...
            <div style={{ overflowY: "auto", padding: "20px 24px", display: "flex", flexDirection: "column", gap: 20 }}>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
                {[{ label: "平均回答スコア", val: avgScore, icon: "🎯" }, { label: "平均アドポリ適合", val: avgAdpoli, icon: "📋" }, { label: "回答問数", val: allFeedbacks.length, icon: "💬" }].map(({ label, val, icon }) => (
                  <div key={label} style={{ padding: "16px", border: "1px solid #e2e8f0", borderRadius: 12, textAlign: "center" }}>
                  ...
                  <div key={i} style={{ border: "1px solid #e2e8f0", borderRadius: 12, overflow: "hidden" }}>
                    <div style={{ padding: "8px 16px", background: "linear-gradient(135deg,rgba(124,58,237,.08),rgba(37,99,235,.08))", borderBottom: "1px solid #e2e8f0", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                      ...
                      <div style={{ display: "flex", gap: 8 }}>
                        <span style={{ fontSize: 11, padding: "2px 8px", borderRadius: 5, background: "rgba(124,58,237,.1)", color: "#7c3aed", fontFamily: "monospace" }}>スコア: {fb.score}</span>
                        <span style={{ fontSize: 11, padding: "2px 8px", borderRadius: 5, background: "rgba(37,99,235,.1)", color: "#2563eb", fontFamily: "monospace" }}>適合: {fb.adpoli_match}</span>
                      </div>
                    </div>
              ...
              <div style={{ display: "flex", gap: 12, justifyContent: "center", paddingTop: 8 }}>
                <button onClick={() => downloadPDF(univ, messages, allFeedbacks)}
                  style={{ padding: "12px 28px", background: "linear-gradient(135deg,#7c3aed,#2563eb)", border: "none", borderRadius: 11, color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", boxShadow: "0 4px 14px rgba(124,58,237,.35)" }}>
                  📄 PDFレポートをダウンロード
                </button>
                <button onClick={startInterview}
      ...
      {(phase === "interview" || phase === "done") && univ && (
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "20px 24px", display: "grid", gridTemplateColumns: "1fr 380px", gap: 20, alignItems: "start" }}>
            ...
            <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 14, overflow: "hidden", boxShadow: "0 1px 6px rgba(0,0,0,.06)" }}>
              ...
              {phase === "interview" && (
                <div style={{ borderTop: "1px solid #e2e8f0", padding: "14px 16px", display: "flex", gap: 10 }}>
                  ...
                  <button onClick={sendAnswer} disabled={loading || !input.trim()}
                    style={{ padding: "0 20px", background: loading || !input.trim() ? "#e2e8f0" : "linear-gradient(135deg,#7c3aed,#2563eb)", border: "none", borderRadius: 10, color: loading || !input.trim() ? "#94a3b8" : "#fff", fontSize: 20, cursor: loading || !input.trim() ? "not-allowed" : "pointer" }}>
                    ↑
                  </button>
                </div>
              )}
              ...
              {phase === "done" && (
                <div style={{ borderTop: "1px solid #e2e8f0", padding: "20px", textAlign: "center", background: "#fafbff" }}>
                  ...
                  <div style={{ fontSize: 13, color: "#64748b", marginBottom: 16 }}>ヘッダーの「まとめを見る」からPDFレポートをダウンロードできます。</div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14, position: "sticky", top: 20 }}>
            <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 14, overflow: "hidden", boxShadow: "0 1px 6px rgba(0,0,0,.06)" }}>
              <div style={{ padding: "10px 16px", borderBottom: "1px solid #e2e8f0", fontSize: 11, color: "#64748b", fontFamily: "monospace", letterSpacing: ".07em", textTransform: "uppercase" }}>📋 この大学が重視すること</div>
            ...
            {allFeedbacks.length > 0 && (
              <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 14, overflow: "hidden", boxShadow: "0 1px 6px rgba(0,0,0,.06)" }}>
                <div style={{ padding: "10px 16px", borderBottom: "1px solid #e2e8f0", fontSize: 11, color: "#64748b", fontFamily: "monospace", letterSpacing: ".07em", textTransform: "uppercase" }}>📈 現在の平均スコア</div>
            ...
            <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 14, overflow: "hidden", boxShadow: "0 1px 6px rgba(0,0,0,.06)" }}>
              <div style={{ padding: "10px 16px", borderBottom: "1px solid #e2e8f0", fontSize: 11, color: "#64748b", fontFamily: "monospace", letterSpacing: ".07em", textTransform: "uppercase" }}>💡 リアルタイムフィードバック</div>
              ...
              {!feedbackLoading && !feedback && (
                <div style={{ padding: "24px 16px", textAlign: "center", color: "#94a3b8", fontSize: 13 }}>回答すると<br />フィードバックが表示されます</div>
              )}
              ...
              {!feedbackLoading && feedback && (
                <div style={{ padding: "14px 16px", display: "flex", flexDirection: "column", gap: 12 }}>
                  ...
                  <button onClick={retryLastQuestion}
                    style={{ width: "100%", padding: "10px", background: "#fff", border: "2px solid #7c3aed", borderRadius: 9, color: "#7c3aed", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
                    🔄 もう一回チャレンジ
                  </button>
                  <button onClick={startInterview}
                    style={{ width: "100%", padding: "10px", background: "#fff", border: "1px solid #e2e8f0", borderRadius: 9, color: "#64748b", fontSize: 13, cursor: "pointer", fontFamily: "inherit", marginTop: 6 }}>
                    ↩️ 初めからやり直す
                  </button>
